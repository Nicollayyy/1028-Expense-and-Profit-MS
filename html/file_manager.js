...
(function () {
  ...
  filtered.forEach(f => {
          ...
          aDown.setAttribute('title', 'Download');
          aDown.setAttribute('aria-label', 'Download');
          // grid view: compact icon + small label to match requested style
          aDown.innerHTML = '<i class="fas fa-download" aria-hidden="true"></i> <span class="btn-label">Download</span>';
          const btnDel = document.createElement('button');
          btnDel.className = 'btn-delete';
          btnDel.type = 'button';
          btnDel.setAttribute('title', 'Delete');
          btnDel.setAttribute('aria-label', 'Delete');
          // grid view: compact delete button with label
          btnDel.innerHTML = '<i class="fas fa-trash-alt" aria-hidden="true"></i> <span class="btn-label">Delete</span>';
      ...
      function renderTable(filesArr, filter) {
        ...
        filtered.forEach(f => {
          ...
          const aDown = document.createElement('a');
          aDown.href = url; aDown.setAttribute('download', f.name); aDown.className = 'btn-download'; aDown.setAttribute('title','Download'); aDown.setAttribute('aria-label','Download'); aDown.innerHTML = '<i class="fas fa-download" aria-hidden="true"></i> <span class="btn-label">Download</span>';
          const btnDel = document.createElement('button'); btnDel.className = 'btn-delete'; btnDel.type = 'button'; btnDel.setAttribute('title','Delete'); btnDel.setAttribute('aria-label','Delete'); btnDel.innerHTML = '<i class="fas fa-trash-alt" aria-hidden="true"></i> <span class="btn-label">Delete</span>';
          btnDel.addEventListener('click', async () => {
            if (!confirm('Delete "' + f.name + '" ?')) return;
            try {
              const res = await fetch('../delete_file.php', { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded' }, body: 'name=' + encodeURIComponent(f.name) });
